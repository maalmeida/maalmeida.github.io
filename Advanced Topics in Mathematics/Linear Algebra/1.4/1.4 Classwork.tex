...
\flushleft
If $A$ is an $m \times n$ matrix, with columns $\mathbf{a}_1, \ldots, \mathbf{a}_n$, and if $\mathbf{x} \in \mathbb{R}^n$, then  \textbf{the product of $A$ and $\mathbf{x}$, denoted $A\mathbf{x}$}, is the \textbf{linear combination of the columns of $A$ using the corresponding entries in $\mathbf{x}$ as weights}; that is,
\begin{equation*}
