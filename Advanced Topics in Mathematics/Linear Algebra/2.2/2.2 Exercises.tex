...
\begin{questions}
    \uplevel{Find the inverses of the matrices in Exercises 1 - 4. Then, vertify your proposed inverse.}
    \question $\mat{8 & 3 \\ 5 & 2}$
    \question $\mat{3 & 1 \\ 7 & 2}$
    \question $\mat{8 & 3 \\ -7 & -3}$
    \question $\mat{3 & -2 \\ 7 & -4}$
    \question Skip
    \question Skip
    ...
    \question (\textbf{T}/\textbf{F}) In order for a matrix $B$ to be the inverse of $A$, btoh equations $AB = I$ and $BA = I$ must be true.
    ...
    \vspace{1cm}
    \question Let $A$ be an invertible $n times n$ matrix, and let $B$ be an $n \times p$ matrix. Explain why $A^{-1}B$ can be computed by row reduction:
